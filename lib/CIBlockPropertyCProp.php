...
<?
    ...
    {
        $val = '';
        if (isset($arValue['VALUE'][$code])) {
            $val = $arValue['VALUE'][$code];
        }

        $inputName = $strHTMLControlName['VALUE'] . '[' . $code . ']';

        $editorId = 'editor_' . md5($inputName . mt_rand());
        ...
        return '<tr>
            <td align="right" valign="top" style="padding-top: 10px;">' . htmlspecialcharsbx($title) . ':</td>
            <td>' . $editorHtml . '</td>
        </tr>';
    }
}
